...
def collect_means_from_pickles(pkl_files):
    ...
    for pkl_file in pkl_files:
        ...
        # Extract mean number of inliers for RANSAC_109
        ransac_109_inliers = data.get('mean_number_inliers_standard_RANSAC_109', 0)

        for key in data.keys():
            if key.startswith('mean_number_inliers') and 'standard_RANSAC_109' not in key:
                ...
                if transformed_key not in improvement_data:
                    improvement_data[transformed_key] = []
                
                # Calculate percentage improvement over RANSAC_109
                current_inliers = data[key]
                percentage_improvement = ((current_inliers - ransac_109_inliers) / ransac_109_inliers) * 100
                improvement_data[transformed_key].append(percentage_improvement)
...
plot_boxplots_with_annotations_seaborn(percentage_data, save_path='boxplots_percentage_improvement_open3d.png')
